...
class PerformanceDialogMixin:
    def show_performance_report(self):
        ...
        tk.Label(
            hdr, text="📈", bg="#050E0A", fg="#10B981", font=(self.display_font, 14), width=4, pady=14
        ).pack(side="left")
        tk.Label(
            hdr, text="Performance Report", bg=P["surface_alt"], fg=P["text"], font=(self.display_font, 14)
        ).pack(side="left", padx=(4, 0))
        tk.Label(
            hdr,
            text="Live session metrics & task statistics",
            bg=P["surface_alt"],
            fg=P["muted"],
        ...
        for i, (key, label, val, color, _) in enumerate(kpis):
            card = tk.Frame(
                kpi_row, bg=P["surface_alt"], highlightthickness=1, highlightbackground=P["border"]
            )
            ...
            val_lbl = tk.Label(
                inner, text=val, bg=P["surface_alt"], fg=color, font=(self.display_font, 22), anchor="w"
            )
        ...
        # section label
        tk.Label(
            body, text="RESOURCE UTILIZATION", bg=P["surface"], fg=P["muted"], font=(self.display_font, 9)
        ...
        for key, label, (c1, c2), init in bars:
            ...
            top.pack(fill="x")
            tk.Label(
                top, text=label, bg=P["surface"], fg=P["muted"], font=(self.mono_font, 9), anchor="w"
            ).pack(side="left")
            val_lbl = tk.Label(top, text=init, bg=P["surface"], fg=c2, font=(self.mono_font, 9))
            val_lbl.pack(side="right")

            bar = GradientProgressBar(row, bg=P["surface_alt"], color_start=c1, color_end=c2, height=5)
        ...
        def _btn(text, fg, side, cmd):
            f = tk.Frame(fp, bg=fg if side == "right" else P["border"], padx=1, pady=1)
