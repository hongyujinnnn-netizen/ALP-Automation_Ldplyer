...
class SchedulePageMixin:
    ...
    def create_enhanced_schedule(self, parent):
        ...
        tb.Label(time_frame, text="Schedule Time:", bootstyle="secondary", width=15).pack(side="left", padx=5)

        # Time entry with better UI
        time_entry = tb.Entry(time_frame, textvariable=self.schedule_time, width=10, font=("Segoe UI", 11))
        time_entry.pack(side="left", padx=5)

        # Add time picker button
        tb.Button(time_frame, text="Pick Time", command=self.show_time_picker, bootstyle="secondary").pack(
            side="left", padx=5
        )
        ...
        tb.Label(repeat_frame, text="Repeat Every:", bootstyle="secondary", width=15).pack(
            side="left", padx=5
        )

        repeat_spinbox = tb.Spinbox(
            repeat_frame, from_=0, to=24, textvariable=self.schedule_repeat_hours, width=5
        )
        ...
        # Schedule type
        type_frame = tb.Labelframe(schedule_frame, text="Schedule Type", bootstyle="secondary", padding=10)
        ...
        self.days_frame = tb.Labelframe(
            schedule_frame, text="Days of Week", bootstyle="secondary", padding=10
        )
        ...
        self.schedule_enable_btn = tb.Button(
            control_frame, text="Enable Schedule", command=self.toggle_schedule, bootstyle="success", width=25
        )
        self.schedule_enable_btn.pack()

        # Next run info
        self.next_run_label = tb.Label(control_frame, text="Next run: Not scheduled", bootstyle="secondary")
