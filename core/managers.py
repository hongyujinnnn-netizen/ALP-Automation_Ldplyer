...
class AccountManager:
    ...
    def save_accounts(self) -> None:
        ...
        for account in self.accounts:
            ...
            if not account_id:
                if non_empty:
                    logger.warning(
                        "[credential-migration] account in %s has no account_id; secrets remain as plaintext",
                        self.accounts_file,
                    )
        ...
        for account in accounts:
            ...
            if not account_id:
                if plaintext_present:
                    logger.warning(
                        "[credential-migration] account in %s has no account_id; leaving plaintext as-is",
                        self.accounts_file,
                    )
                continue
            if plaintext_present:
    ...
    def get_device_account(self, device_name: str) -> dict:
        ...
        matches = [
            ...
            for account in self.accounts
            if str(
                account.get("device_name") or account.get("instance") or account.get("ld_name") or ""
            ).strip()
            == device_name
        ]
        if not matches:
    ...
    def import_accounts(self, file_path: str | Path) -> int:
        ...
        for record in records:
            ...
            if existing_identifier and any(
                self._get_account_identifier(row) == existing_identifier for row in self.accounts
            ):
    ...
    def _normalize_account_record(self, account_data: dict, existing: dict | None = None) -> dict:
        ...
        adb_serial = str(clean.get("ld_adb") or existing.get("ld_adb") or "").strip()
        ...
        facebook_uid = str(clean.get("facebook_uid") or existing.get("facebook_uid") or "").strip()
        account_id = str(
    ...
    def load_content_queue(self):
        if self.content_file.exists():
            try:
                ...
                corrupt_copy = self.content_file.with_suffix(
                    f"{self.content_file.suffix}.corrupt_{timestamp}"
                )
                try:
...
class SmartScheduler:
    ...
    def should_run_task(self, task, current_time):
        ...
        if task_time.hour == current_time_time.hour and task_time.minute == current_time_time.minute:
        ...
        for task in self.task_queue:
            if task["id"] == task_id:
                ...
                if task["repeat_interval"]:
                    task["next_run"] = self.calculate_next_run(task["schedule_time"], task["repeat_interval"])
                break
        self.save_scheduled_tasks()
    ...
    def create_backup(self, include_logs=True, include_content=True):
        ...
        try:
            with zipfile.ZipFile(backup_file, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                # Backup logs
                if include_logs:
                    log_files = list(self.paths.project_root.glob("*.log")) + list(
                        self.paths.logs_dir.glob("*.log")
                    )
...
class TaskTemplates:
    TEMPLATES = {
        ...
        "content_day": {
            "name": "Content Day",
            "tasks": [{"type": "reels", "max_videos": 10, "scroll_after_post": True}],
